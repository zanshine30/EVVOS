...
# mic_shared  — stereo dsnoop (multiple readers, no channel conversion)
# mic_mono    — plug wrapper: stereo dsnoop → mono 16 kHz
#               arecord must use mic_mono, not mic_shared, because dsnoop
#               is a pass-through multiplexer and rejects channel-count
#               mismatches ("Channels count non available" error).
# pcm.!default — routes ALSA default capture → mic_shared so PortAudio
#               (PyAudio) gets dsnoop when it opens the default device.

cat > /etc/asound.conf << ASOUND_EOF
# ── EVVOS Shared Audio — generated by setup_shared_audio.sh ───────────────
# Allows evvos-pico-voice and evvos-picam-tcp to read the ReSpeaker mic
# simultaneously via ALSA dsnoop (software capture multiplexer).

# ── Stereo dsnoop — raw multiplexed stream ────────────────────────────────
pcm.mic_shared {
    type dsnoop
    ipc_key 2048
...
# ── Mono wrapper — stereo dsnoop → mono 16 kHz ───────────────────────────
# Used by arecord in the camera service.
# dsnoop cannot do channel conversion; the plug layer handles it.
pcm.mic_mono {
    type plug
    slave {
        pcm "mic_shared"
...
# Verify dsnoop with services stopped (hw device is free)
log_info "Testing dsnoop with services stopped..."
DSNOOP_ERR=$(mktemp)
if timeout 4 arecord -D mic_shared -f S16_LE -r 16000 -c 2 -d 2 /tmp/test_dsnoop.wav 2>"$DSNOOP_ERR"; then
    ...
    log_error "dsnoop test failed: $(cat "$DSNOOP_ERR")"
    log_error "Check card number in /etc/asound.conf (currently: $CARD_NUM)"
    rm -f "$DSNOOP_ERR"
    exit 1
fi
rm -f "$DSNOOP_ERR"

log_info "Testing mic_mono (stereo→mono plug wrapper)..."
MONO_ERR=$(mktemp)
if timeout 4 arecord -D mic_mono -f S16_LE -r 16000 -c 1 -d 2 /tmp/test_mono.wav 2>"$MONO_ERR"; then
    ...
    [ "$SIZE" -gt 20000 ] \
        ...
        || log_warning "mic_mono opened but file small (${SIZE}B)"
else
    log_error "mic_mono test failed: $(cat "$MONO_ERR")"
    rm -f "$MONO_ERR"
    exit 1
fi
rm -f "$MONO_ERR"
