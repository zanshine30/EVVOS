...
# WHY two layers (dsnoop + plug) instead of one?
#   The TLV320AIC3104 codec runs at a fixed native rate (usually 48000 Hz).
#   Configuring dsnoop's slave at 16000 Hz causes the hardware to reject it
#   ("Slave PCM not usable"). Fix: dsnoop runs at native rate; a plug wrapper
#   resamples down to 16000 Hz.
#
# WHY no mic_mono device?
#   dsnoop rejects any connection whose channel count doesn't match its slave
#   config (channels 2). A plug wrapping dsnoop with channels 1 still tries
#   to open dsnoop with 1 channel — dsnoop refuses ("Broken configuration").
#   Solution: arecord records stereo (-c 2) from mic_shared, and ffmpeg
#   downmixes to mono during AAC encoding (-ac 1). No ALSA channel
...
# dsnoop runs at native rate; mic_shared plug resamples to 16000 Hz stereo.
# arecord records stereo; ffmpeg downmixes to mono during AAC encoding.
...
# ── Layer 2: stereo 16 kHz — used by both voice service and arecord ───────
# Both consumers open this device with -c 2 (stereo).
# Voice service (Rhino) uses channel 0 internally.
# arecord records a stereo WAV; ffmpeg downmixes L+R → mono with -ac 1
# during AAC encoding. No ALSA-level channel conversion needed.
pcm.mic_shared {
    type plug
    slave {
        pcm "mic_dsnoop"
...
log_success "/etc/asound.conf written (dsnoop @ ${HW_RATE} Hz → mic_shared @ 16000 Hz stereo)"

# ── Test mic_shared with stereo (the only device we now use) ─────────────────
...
if timeout 4 arecord -D mic_shared -f S16_LE -r 16000 -c 2 -d 2 /tmp/test_shared.wav 2>"$SHARED_ERR"; then
    ...
    log_error "Detected rate was ${HW_RATE} Hz — run: arecord -D hw:${CARD_NUM},0 --dump-hw-params"
    rm -f "$SHARED_ERR"
    exit 1
fi
rm -f "$SHARED_ERR"
...
echo ""
echo "  ALSA devices:"
echo "    mic_dsnoop  — stereo ${HW_RATE} Hz raw dsnoop (hardware lock)"
echo "    mic_shared  — stereo 16 kHz plug→dsnoop (voice service + arecord)"
