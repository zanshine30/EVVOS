...
#
# Uses regex to locate the closing brace of get_status_handler's return dict
# regardless of indentation style or whether elapsed is inlined or a variable.
# ─────────────────────────────────────────────────────────────────────────────
if not p2_done:
    # Find get_status_handler in src, then find its return dict's closing brace
    func_start = src.find("def get_status_handler():")
    assert func_start != -1, "get_status_handler not found in file"

    # Search only within get_status_handler's body (next 800 chars is plenty)
    window = src[func_start: func_start + 800]

    # Match the last "elapsed_seconds" line + the closing brace that follows it
    m = re.search(
        r'("elapsed_seconds"[^\n]+\n)(\s+\})',
        window
    )
    assert m, (
        "Patch 2: could not locate closing brace after 'elapsed_seconds' in "
        "get_status_handler.\nWindow content:\n" + window
    )

    indent     = m.group(2)[:-1]   # whitespace before }  e.g. "            "
    old_frag   = m.group(0)        # the matched text inside the window
    new_frag   = m.group(1) + indent + '"battery":         get_battery_level(),\n' + m.group(2)

    # Replace only within get_status_handler's window to avoid touching other dicts
    new_window = window.replace(old_frag, new_frag, 1)
    src = src[:func_start] + new_window + src[func_start + 800:]
