...
"""
        ...
        if self.credentials:
            ...
            if connection_established:
                ...
                for attempt in range(1, 6):
                    ...
                    if await self._check_internet():
                        logger.info("✓ Internet connection verified!")
                        # User ID is already stored, no need to re-report on reboot unless desired
                        # If you want to re-verify/update on every boot, uncomment below:
                        # user_id = self.credentials.get("user_id", "")
                        # await self._report_to_supabase(ssid, password, user_id=user_id)
                        return True
                    await asyncio.sleep(3)
    ...
    async def run(self):
        ...
        # Start background connectivity monitor (sends heartbeats every 5 minutes, checks for disconnect)
