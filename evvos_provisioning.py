...
class EVVOSWiFiProvisioner:
    ...
    async def _check_disconnect_requested(self) -> bool:
        ...
        try:
            ...
            # Query the device_credentials table for unpair_requested flag
            url = f"{SUPABASE_URL}/rest/v1/device_credentials"
            params = {
                "user_id": f"eq.{user_id}",
                "device_id": f"eq.{device_id}",
                "select": "unpair_requested,unpair_requested_at"
            }
                ...
                async with session.get(url, params=params, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status == 200:
                        ...
                        if data and len(data) > 0:
                            record = data[0]
                            if record.get('unpair_requested') == True:
                                logger.info(f"[DISCONNECT] Unpair request detected at {record.get('unpair_requested_at')}")
                                return True
        except Exception as e:
        ...
        try:
            ...
            if user_id:
                try:
                    ...
                    # Update the record to clear unpair_requested, clear credentials, and set status
                    payload = {
                        ...
                        "unpaired_at": self._get_manila_time().isoformat(),
                    }
...
"""
    ...
    async def _diagnose_device_credentials(self, user_id: str) -> None:
        ...
        Called when heartbeat fails to help identify query filter mismatches.
        """
        try:
            ...
            # First, try exact match on both user_id and device_id
            params = {
                "device_id": f"eq.{self.device_id}",
                "select": "id,user_id,device_id,device_name,device_status,last_seen,created_at",
            }
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.patch(
                    ...
                    if resp.status in [200, 204]:
                        # CRITICAL: Check if rows were actually updated
                        # Supabase returns 200 even if no rows match the filter
                        try:
                            response_data = await resp.json()
                            # If using Prefer: return=representation, response should be non-empty array
                            if isinstance(response_data, list) and len(response_data) > 0:
                                logger.debug(f"✓ Device heartbeat sent - last_seen updated: {response_data[0].get('last_seen')}")
                                return True
                            else:
                                # No rows updated - query filter didn't match any rows
                                logger.warning(f"[HEARTBEAT] ❌ Query returned no matching rows!")
                                ...
                                # Run diagnostic to see what's actually there
                                await self._diagnose_device_credentials(user_id)
                                return False
                        except Exception as json_error:
                            logger.warning(f"[HEARTBEAT] Could not parse response: {json_error}")
                            # If resp.status is 204, there's no content, which might actually be OK
                            if resp.status == 204:
                                logger.warning(f"[HEARTBEAT] Got 204 No Content - unable to verify if rows were updated")
                                logger.warning(f"[HEARTBEAT] This may indicate Supabase RLS policy issues or the row doesn't exist")
                                await self._diagnose_device_credentials(user_id)
                                return False
                            return False
                    else:
