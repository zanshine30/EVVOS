#!/bin/bash
# Patch for Flask app.py - Remove sudo calls from subprocess (app runs as root)
# Usage: sudo bash -c "$(curl -fsSL https://raw.githubusercontent.com/shyroldan/EVVOS/refs/heads/main/FIX_SUDO_ISSUE.sh)"

echo "🔧 Patching Flask app.py - fixing sudo issue..."
...
'''
        ...
        config_path = "/etc/wpa_supplicant/wpa_provisioned.conf"
...
"""
        ...
        # App runs as root via systemd, no sudo needed
        subprocess.run(['mv', '/tmp/wpa_temp.conf', config_path], check=True)
        subprocess.run(['chmod', '600', config_path], check=True)
...
echo "✅ app.py patched - sudo calls removed"
...
echo ""
echo "✨ Patch applied! Try sending credentials again."
