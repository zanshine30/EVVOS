...
# The TLV320AIC3104 codec supports a rate range [8000 96000], meaning 16000 Hz
# works natively on the hardware. We set dsnoop directly to 16000 Hz — no plug
# resampling layer needed. Previous attempts auto-detected the "max" rate (96000)
# and wrapped dsnoop in a plug to resample down, which caused "Slave PCM not
# usable" errors. Simplest fix: use 16000 Hz directly in dsnoop.
#
# arecord records stereo (-c 2) because dsnoop only accepts connections that
# match its configured channel count. ffmpeg downmixes stereo→mono with -ac 1
# during AAC encoding — no ALSA channel conversion needed.

# ── Verify 16000 Hz is natively supported ────────────────────────────────────
log_info "Verifying hw:${CARD_NUM},0 supports 16000 Hz S16_LE stereo..."
if timeout 3 arecord -D hw:${CARD_NUM},0 -f S16_LE -r 16000 -c 2 -d 1 /tmp/hw_test.wav 2>/dev/null; then
    log_success "Confirmed: 16000 Hz S16_LE stereo works natively on hw:${CARD_NUM},0"
    rm -f /tmp/hw_test.wav
else
    log_warning "Direct hw test inconclusive (may just be arecord timeout) — proceeding"
    rm -f /tmp/hw_test.wav
fi

cat > /etc/asound.conf << ASOUND_EOF
# ── EVVOS Shared Audio — generated by setup_shared_audio.sh ───────────────
# dsnoop runs at 16000 Hz directly (hardware supports range [8000, 96000]).
# No plug/resampling layer needed.
# arecord records stereo WAV; ffmpeg downmixes to mono AAC with -ac 1.

# ── Shared capture device: 16000 Hz stereo dsnoop ────────────────────────
# Multiple processes open this simultaneously — no exclusive hw lock.
pcm.mic_shared {
    type dsnoop
    ipc_key 2048
...
# ── System default ────────────────────────────────────────────────────────
# Capture → mic_shared; playback → card speaker.
# PyAudio uses this when input_device_index=None.
pcm.!default {
    type asym
    playback.pcm {
        type plug
...
log_success "/etc/asound.conf written (dsnoop directly at 16000 Hz stereo)"

# ── Test mic_shared ───────────────────────────────────────────────────────────
log_info "Testing mic_shared (stereo 16 kHz)..."
SHARED_ERR=$(mktemp)
if timeout 4 arecord -D mic_shared -f S16_LE -r 16000 -c 2 -d 2 /tmp/test_shared.wav 2>"$SHARED_ERR"; then
    SIZE=$(stat -c%s /tmp/test_shared.wav 2>/dev/null || echo 0)
    rm -f /tmp/test_shared.wav
    [ "$SIZE" -gt 20000 ] \
    ...
    log_error "mic_shared failed: $(cat "$SHARED_ERR")"
    log_error "Try manually: arecord -D hw:${CARD_NUM},0 -f S16_LE -r 16000 -c 2 -d 2 /tmp/test.wav"
    rm -f "$SHARED_ERR"
    exit 1
fi
rm -f "$SHARED_ERR"
...
NEW_START = '''def start_recording_handler():
    ...
    with recording_lock:
        ...
        try:
            ...
            audio_cmd = [
                "arecord",
                "-D", "mic_shared",  # dsnoop shared device (stereo 16 kHz)
                "-f", "S16_LE",
                "-r", "16000",
                "-c", "2",           # stereo — dsnoop requires matching channel count
...
ARECORD_ERR=$(mktemp)
if timeout 5 arecord -D mic_shared -f S16_LE -r 16000 -c 2 -d 3 /tmp/sim_test.wav 2>"$ARECORD_ERR"; then
...
echo ""
echo "  ALSA devices:"
echo "    mic_shared  — stereo 16 kHz dsnoop directly on hw:${CARD_NUM},0"
echo "    Stereo→mono downmix handled by ffmpeg (-ac 1) during MP4 encoding"
echo ""
...
echo "  Test mic_shared: arecord -D mic_shared -c 2 -d 3 /tmp/test.wav && aplay /tmp/test.wav"
