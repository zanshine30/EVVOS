...
"""
    ...
    async def _update_device_heartbeat(self, user_id: str = None) -> bool:
        ...
        This serves as a keep-alive signal to indicate the device is online.
        """
        try:
            if not user_id:
            ...
            # Update the device_credentials table with last_seen timestamp only
            url = f"{SUPABASE_URL}/rest/v1/device_credentials"
            ...
            async with aiohttp.ClientSession() as session:
                async with session.patch(
                    ...
                    if resp.status in [200, 204]:
                        logger.debug(f"✓ Device heartbeat sent - last_seen updated")
                        return True
                    else:
                        error_text = await resp.text()
                        logger.debug(f"Failed to send heartbeat: {resp.status} - {error_text}")
                        return False
        except Exception as e:
            logger.debug(f"Error sending device heartbeat: {e}")
            return False
...
"""
    ...
    async def _start_http_server(self) -> None:
        """Start aiohttp server for credential submission"""
        try:
            app = web.Application()
            ...
            logger.info("✓ HTTP credential server started on 0.0.0.0:8000")
    ...
    async def _provision_with_hotspot(self):
        ...
        try:
            ...
            if await self._wait_for_hotspot_credentials(timeout_seconds=600):
                ...
                logger.info("Waiting 15 seconds for mobile app to confirm provisioning...")
                ...
                if await self._report_to_supabase(ssid, password, user_id=user_id):
                    ...
                    # Send initial heartbeat to set status='online' and last_seen timestamp
                    if user_id:
            ...
            if connection_established:
                ...
                for attempt in range(1, 6):
                    ...
                    if await self._check_internet():
                        ...
                        # Send heartbeat to set status='online' and update last_seen
    ...
    async def _monitor_connectivity(self) -> None:
        """
        Background task that monitors device connectivity.
        Sends periodic heartbeats (every 5 minutes) to keep last_seen timestamp updated.
        If connectivity is lost, the device stops sending heartbeats and the
        last_seen timestamp becomes stale, indicating the device is offline.
        """
        try:
            while True:
                ...
                if self.credentials and self.credentials.get("user_id"):
                    ...
                    if await self._check_internet():
                        ...
                        logger.debug("No internet connectivity - heartbeat skipped")
        except Exception as e:
    ...
    async def run(self):
        ...
        # Start background connectivity monitor (sends heartbeats every 5 minutes)
...
echo "✓ Provisioning script deployed (1260 lines, production-ready)"
