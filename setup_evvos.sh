...
# Configure network for stability (DNS + pip timeout)
echo "Configuring DNS and network stability..."
mkdir -p /root/.pip
...
prefer-binary = true
PIPCONFIG
echo "✓ Pip timeout set to 1000s, binary preference enabled"

# Update DNS to Google's resolver for stability
if ! grep -q "nameserver 8.8.8.8" /etc/resolv.conf; then
  echo "nameserver 8.8.8.8" >> /etc/resolv.conf
  echo "✓ DNS set to 8.8.8.8 for stable pip downloads"
fi
...
pip install --no-cache-dir --prefer-binary numpy==1.26.4 --timeout=1000

# Install Vosk and audio libraries with binary preference
...
echo ""
echo "📋 Step 5.5: Create Voice Command Service"
echo "=========================================="

# Create placeholder for voice command service
# This service will be started/stopped by the provisioning script
# based on provisioning state
mkdir -p /usr/local/bin

cat > /usr/local/bin/evvos-voice-command << 'VOICE_COMMAND_EOF'
#!/usr/bin/env python3
"""
EVVOS Voice Command Service with ReSpeaker 2-Mics Pi HAT v2.0
Uses Vosk for lightweight speech recognition with grammar optimization
Controlled by provisioning system - starts after successful WiFi provisioning
"""
...
MODEL_PATH = "/opt/evvos/vosk-model"
...
class VoiceCommandService:
    ...
    def _setup_vosk(self):
        try:
            ...
            try:
                ...
                logger.warning("Using default Vosk language model")
                return False
    ...
    def _process_command(self, text):
        ...
        for cmd in self.commands:
            if cmd.lower() in text_lower:
                logger.info(f"Command matched: {cmd}")
                timestamp = datetime.now(self.manila_tz).isoformat()
                log = {"timestamp": timestamp, "command": cmd}
                logger.info(f"Executed: {json.dumps(log)}")
                break
    ...
    def run(self):
        logger.info("="*60)
        logger.info("EVVOS Voice Command Service with ReSpeaker HAT")
        logger.info("="*60)
...
chmod +x /usr/local/bin/evvos-voice-command
echo "✓ Voice command script created"

# Create systemd service file for voice command
cat > /etc/systemd/system/evvos-voice-command.service << 'VOICE_SERVICE_FILE'
[Unit]
Description=EVVOS Voice Command Service
...
User=root
WorkingDirectory=/opt/evvos
ExecStart=/usr/local/bin/evvos-voice-command
Restart=no
StandardOutput=journal
StandardError=journal
...
echo "  • Vosk recognizes 8 commands: okay, confirm, cancel, start, stop, help, repeat, clear"
echo ""
